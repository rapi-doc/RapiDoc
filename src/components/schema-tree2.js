...
export default class SchemaTree2 extends LitElement {
  ...
  generateTree(data, prevDataType = 'object') {
    ...
    if (typeof data === 'object') {
      ...
      let openBracket;
      let closeBracket;
      if (Object.keys(data)[0].startsWith('::OPTION') || Object.keys(data)[0] === '::ANY~OF' || Object.keys(data)[0] === '::ONE~OF') {
        if (['::ANY~OF', '::ONE~OF'].includes(Object.keys(data)[0])) {
              ...
              : html`
                <div class="item"> 
                  <span class="item__key"> 
                    ${['::ANY~OF', '::ONE~OF'].includes(key)
                      ? html`
                        <span class="any-of-one-of" >${key.replace('~', ' ').replace('::', '')}</span>`
                      : key.startsWith('::OPTION')
                        ? html`<span class="any-of-one-of" >${key.replace('::OPTION~', ' ')}</span>`
                        : key.endsWith('*')
...
customElements.define('schema-tree2', SchemaTree2);
