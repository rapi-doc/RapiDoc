...
export default class ApiRequest extends LitElement {
  ...
  static get properties() {
    return {
      ...
      selectedRequestBodyType: { type: String, attribute: 'selected-request-body-type' }, // internal tracking of request-body type
    };
  }
  ...
  requestBodyTemplate() {
    ...
    for (const mimeReq in content) {
      ...
      if (!this.selectedRequestBodyType) {
        this.selectedRequestBodyType = mimeReq;
      }
      mimeReqCount++;
    }
    return html`
                    ...
                    : html`
                      ${Object.keys(shortMimeTypes).map((k) => html`
                        <input type='radio' 
                          ...
                          ?checked = '${shortMimeTypes[k] === this.selectedRequestBodyType}'
  ...
  onMimeTypeChange(e) {
    ...
    // this.selectedRequestBodyType = e.target.value;
    this.setAttribute('selected-request-body-type', this.shadowRoot.getElementById('spec-url').value);
  ...
  async onTryClick(e) {
    ...
    if (formParamEls.length >= 1) {
      ...
      if (formEl.classList.contains('form')) {
      ...
      if (dynFormParamEls[0].classList.contains('form')) {
        fetchOptions.headers['Content-Type'] = 'application/x-www-form-urlencoded; charset=utf-8';
        curlHeaders += ' -H "Content-Type: application/x-www-form-urlencoded"';
        fetchOptions.body = formUrlParams;
      } else {
